...
def collate(samples, pad_idx):
    ...
    paragraph_mask = data_utils.collate_tokens(
        [s['paragraph_mask'] for s in samples], pad_idx,  left_pad=False)


    start_target = torch.full(text.size(), 0).long()
    end_target = torch.full(text.size(), 0).long()
    for idx, s in enumerate(samples):
        for start, end in zip(s['target'][0], s['target'][1]):
            start_target[idx, start] = 1
            end_target[idx, end] = 1
...
class SpanQABCEDataset(FairseqDataset):
    """
    ...
    def get_dummy_batch(self, num_tokens, max_positions, tgt_len=128):
        ...
        target = (torch.tensor([self.vocab.pad()]),
                  torch.tensor([self.vocab.pad()]))
        idx_map = [self.vocab.pad()]
        token_is_max_context = [0]
        return self.collater([
            {'id': i, 'text': text, 'target': target, 'segment': segment, 'paragraph_mask': paragraph_mask,
                'squad_ids': 0, 'actual_txt': 'dummy', 'idx_map': idx_map, 'token_is_max_context': token_is_max_context}
            for i in range(bsz)
        ])
