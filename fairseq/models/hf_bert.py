...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        attention_probs = torch.nn.functional.softmax(attention_scores, dim=-1, dtype=torch.float32).type_as(attention_scores)
...
def large_bert_architecture(args):
    args.config = BertConfig()
    args.config.hidden_size = getattr(args, 'hidden_size', 2048)
    args.config.num_hidden_layers = getattr(args, 'num_hidden_layers', 12)
    args.config.num_attention_heads = getattr(args, 'num_attention_heads', 32)
    args.config.intermediate_size = getattr(args, 'intermediate_size', 8192)
    ...
    args.config.initializer_range = getattr(args, 'initializer_range', 0.01)

@register_model_architecture("bert_hf", "hf_wikibook_base")
def hf_wikibook_base_architecture(args):
