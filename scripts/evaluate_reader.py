...
def main(args):
    ...
    with torch.no_grad():
        for batch_ndx, batch_data in enumerate(tqdm(dataloader)):
            ...
            span_scores = outs[0][:,:,None] + outs[1][:,None] # B L L
            ...
            span_scores = span_scores - 1e20 * (1 - span_mask[None].expand_as(span_scores))
            ...
            for qid, doc_tokens, wp_tokens, tok_to_orig_index, start, end, ans_score, r_score, q, c in zip(batch_data['id'], batch_data['doc_tokens'], batch_data['wp_tokens'], batch_data['tok_to_orig_index'], start_position_, end_position_, answer_scores, ranking_scores, batch_data['q'], batch_data['c']):
    ...
    for qid in qid2results.keys():
        qid2results[qid].sort(key=lambda x:x[1] + x[2], reverse=True)
